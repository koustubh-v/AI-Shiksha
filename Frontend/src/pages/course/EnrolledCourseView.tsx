...
export const EnrolledCourseView = () => {
    ...
    return (
        <div className="min-h-screen bg-white font-sans antialiased w-full max-w-full overflow-x-hidden">

            {/* ─── Breadcrumb Nav ─── */}
            <nav className="bg-white border-b border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] w-full">
                <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-2">
                    {/* Breadcrumb — scrollable on tiny screens instead of clipping */}
                    <div className="flex items-center gap-1 sm:gap-1.5 text-sm text-gray-500 min-w-0 overflow-hidden">
                        <Link to="/dashboard" className="hover:text-gray-800 transition-colors whitespace-nowrap flex-shrink-0 text-xs sm:text-sm">
                            Dashboard
                        </Link>
                        <ChevronRight className="h-3 w-3 text-gray-300 flex-shrink-0" />
                        <Link to="/dashboard/my-courses" className="hover:text-gray-800 transition-colors whitespace-nowrap flex-shrink-0 text-xs sm:text-sm hidden xs:block">
                            My Courses
                        </Link>
                        <ChevronRight className="h-3 w-3 text-gray-300 flex-shrink-0 hidden xs:block" />
                        <span className="text-gray-800 font-medium truncate text-xs sm:text-sm">{course.title}</span>
                    </div>
                    {/* Progress pill — only on sm+ */}
                    {hasStarted && (
            ...
            {/* ─── Hero ─── */}
            <section className="relative overflow-hidden bg-gradient-to-br from-[#EEF2FF] via-white to-[#F0F9FF] py-8 md:py-10 w-full">
                {/* Decorative blobs — kept smaller so they don't cause overflow on mobile */}
                <div className="pointer-events-none absolute -top-16 -left-16 w-48 h-48 sm:w-64 sm:h-64 rounded-full bg-blue-100/50 blur-3xl" />
                <div className="pointer-events-none absolute top-10 right-0 w-40 h-40 sm:w-56 sm:h-56 rounded-full bg-indigo-100/40 blur-3xl" />
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
                        ...
                        <div className="space-y-4 sm:space-y-5">
                            ...
                            <div className="space-y-2">
                                <h1 className="text-xl sm:text-2xl md:text-4xl font-extrabold text-[#111827] leading-snug tracking-tight break-words">
                                    {course.title}
                                </h1>
                                {course.subtitle && (
                                    <p className="text-sm sm:text-base md:text-lg text-gray-500 leading-relaxed break-words">
                                        {course.subtitle}
                                    </p>
                                )}
                            </div>

                            {/* Stats — wrap naturally, each stat won't overflow */}
                            <div className="flex flex-wrap gap-3 sm:gap-5 text-sm">
                                {[
                                    ...
                                    <span key={label} className="flex items-center gap-1.5 text-gray-600 font-medium whitespace-nowrap">
                                        <Icon className="h-4 w-4 text-gray-400 flex-shrink-0" />{label}
                                    </span>
                                ))}
                            </div>
                            ...
                            {course.instructor?.user?.name && (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-sm font-semibold text-gray-800 truncate">{course.instructor.user.name}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex gap-3 pt-1 lg:hidden">
                                ...
                                >
                                    ...
                                    <span className="truncate">{hasStarted ? 'Continue Learning' : 'Start Learning'}</span>
                                </Button>
                            </div>
                        </div>
            ...
            {/* ─── Body ─── */}
            <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10 w-full">
                <div className="grid lg:grid-cols-3 gap-6 md:gap-10">

                    {/* ─── Left: main content ─── */}
                    <div className="lg:col-span-2 space-y-6 md:space-y-10 min-w-0">

                        {/* Mobile card — shown only below lg breakpoint */}
                        ...
                        {course.description && (
                            <section>
                                <h2 className="text-lg sm:text-xl font-bold text-[#111827] mb-3">About this course</h2>
                                <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-7 shadow-sm overflow-hidden">
                                    <div
                                        className="prose prose-sm sm:prose-gray max-w-none text-[#374151] text-sm leading-[1.75]
                                                   ...
                                                   [&_*]:max-w-full [&_img]:max-w-full [&_table]:w-full [&_table]:block [&_table]:overflow-x-auto"
                        ...
                        {sections.length > 0 && (
                            <section>
                                <div className="flex items-baseline justify-between mb-4 flex-wrap gap-2">
                                    <h2 className="text-lg sm:text-xl font-bold text-[#111827]">Course Content</h2>
                                    <span className="text-xs sm:text-sm text-gray-400 whitespace-nowrap">
                                        {sections.length} sections · {total} lessons · {totalDuration()}
                                    </span>
                                </div>
                                    ...
                                    {sections.map((section: any, si: number) => (
                                        <div key={section.id} className={si !== 0 ? 'border-t border-gray-100' : ''}>
                                            <button
                                                ...
                                                className="w-full flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 hover:bg-gray-50/80 transition-colors text-left"
                                            >
                                                ...
                                                <div className="bg-gray-50/40 divide-y divide-gray-100/70">
                                                    {(section.items || [])
                                                        ...
                                                        .map((item: any) => {
                                                            ...
                                                            return (
                                                                <div key={item.id} className="flex items-center justify-between px-3 sm:px-6 py-2.5 sm:py-3 hover:bg-white/60 transition-colors gap-2">
                                                                    <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
                                                                        <Icon className="h-4 w-4 text-gray-400 flex-shrink-0" />
                                                                        <span className="text-sm text-gray-700 truncate">{item.title}</span>
                                                                    </div>
                                                                    {item.duration_minutes && (
                                                                        <span className="text-xs text-gray-400 flex-shrink-0">{fmtDuration(item.duration_minutes)}</span>
                                                                    )}
                                                                </div>
                                                            );
                                                        })}
                        ...
                        {course.instructor?.user && (
                            <section>
                                <h2 className="text-lg sm:text-xl font-bold text-[#111827] mb-4">Your Instructor</h2>
                                <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-4 sm:p-5 md:p-7 flex flex-col sm:flex-row items-start gap-4 md:gap-6">
                                    <Avatar className="h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 flex-shrink-0 border-2 border-gray-100 shadow-sm">
                                        {course.instructor.user.avatar_url && <AvatarImage src={course.instructor.user.avatar_url} />}
                                        <AvatarFallback className="bg-[#0056D2] text-white text-lg font-bold">{instructorInitials}</AvatarFallback>
                                    </Avatar>
                                    <div className="min-w-0 w-full">
                                        <h3 className="text-base sm:text-lg font-bold text-[#0056D2] break-words">{course.instructor.user.name}</h3>
                                        <p className="text-sm text-gray-500 mb-2 break-all">{course.instructor.user.email}</p>
                                        {course.instructor.bio && (
                                            <p className="text-sm text-gray-600 leading-relaxed break-words">{course.instructor.bio}</p>
                                        )}
                                    </div>
                                </div>
                            </section>
            ...
            <Dialog open={showTerms} onOpenChange={(o) => { if (!processingTerms) setShowTerms(o); }}>
                {/* max-w capped + mx-4 ensures it never clips on narrow screens */}
                <DialogContent className="w-[calc(100%-2rem)] max-w-2xl p-0 rounded-3xl overflow-hidden gap-0 shadow-2xl border-0 mx-4 sm:mx-auto">
                    {/* Header */}
                    <div className="relative bg-gradient-to-br from-[#0056D2] to-[#0098C7] text-white px-5 sm:px-8 pt-6 sm:pt-8 pb-6 sm:pb-7 overflow-hidden">
                        <div className="absolute -top-8 -right-8 w-40 h-40 bg-white/10 rounded-full" />
                        <div className="absolute -bottom-6 -left-6 w-28 h-28 bg-white/5 rounded-full" />
                        <div className="relative z-10 flex items-start gap-3 sm:gap-4">
                            <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-2xl bg-white/20 flex items-center justify-center flex-shrink-0">
                                <ShieldCheck className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                            </div>
                            <div className="min-w-0">
                                <DialogTitle className="text-lg sm:text-xl font-bold text-white mb-1">Terms & Conditions</DialogTitle>
                    ...
                    {/* Content */}
                    <ScrollArea className="h-[240px] sm:h-[300px] bg-white">
                    ...
                    {/* Footer */}
                    <div className="bg-gray-50 border-t border-gray-100 px-5 sm:px-8 py-4 sm:py-5 space-y-3 sm:space-y-4">
                        ...
                        <div className="flex flex-col xs:flex-row gap-2 sm:gap-3">
                            ...
                            >
                                {processingTerms
                                    ? <><Loader2 className="h-4 w-4 animate-spin flex-shrink-0" /><span>Accepting...</span></>
                                    : <><CheckCircle2 className="h-4 w-4 flex-shrink-0" /><span>Accept &amp; Start</span></>
                                }
                            </Button>
                        </div>
                    </div>
